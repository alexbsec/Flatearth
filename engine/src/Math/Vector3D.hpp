...
public:
  ...
  inline constexpr Vec3D(float32 x, float32 y, float32 z)
      : _x(x), _y(y), _z(z) {}
  inline Vec3D(const Vec3D &other) noexcept
  ...
  inline bool Equals(const Vec3D &other,
  ...
  inline Vec2D XProjection() const noexcept {
    return Vec2D(_x, _z);
  }

  inline Vec2D YProjection() const noexcept {
    return Vec2D(_y, _z);
  }

  inline Vec2D ZProjection() const noexcept {
    return Vec2D(_x, _y);
  }
