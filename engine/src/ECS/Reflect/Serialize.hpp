...
private:
  ...
  FEAPI static std::vector<std::byte> ToBinaryRaw(const void *component,
                                                  const TypeDescriptor &desc);
  FEAPI static bool
  FromBinaryRaw(void *component, const TypeDescriptor &desc, const std::vector<std::byte> &data);
};
