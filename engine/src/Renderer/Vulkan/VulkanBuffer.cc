...
BufferManager::BufferManager(CommandBufferManager &cmdBufferMgr)
    : _cmdBufferManager(cmdBufferMgr) {}

FeExpect<void, Error> BufferManager::CreateVulkanBuffer(
    Context &ctx, uint64 size, VkBufferUsageFlags usage,
    uint32 memoryPropertyFlags, bool bindOnCreate, VulkanBuffer *pBuffer) {
  if (pBuffer == nullptr) {
    FLOG_ERROR("cannot create Vulkan buffer on nullptr");
    return FeErr{
        Error("vulkan buffer is nullptr", ErrorType::NullptrException)};
  }
  ...
  if (auto res = VkCheck(vkCreateBuffer(ctx.device.logicalDevice, &bufferInfo,
                                        ctx.pAllocator, &pBuffer->handle));
      !res.has_value()) {
  ...
  vkGetBufferMemoryRequirements(ctx.device.logicalDevice, pBuffer->handle,
                                &requirements);
  pBuffer->memoryIndex = ctx.FindMemoryIndex(requirements.memoryTypeBits,
                                             pBuffer->memoryPropertyFlags);
  if (pBuffer->memoryIndex == -1) {
    FLOG_ERROR(
        "unable to create vulkan buffer because required memory index was "
        "not found");
    return FeErr{
        Error("failed to find memory index", ErrorType::RendererVulkanError)};
  }
  ...
  VkResult result = vkAllocateMemory(ctx.device.logicalDevice, &allocInfo,
                                     ctx.pAllocator, &pBuffer->memory);
  ...
  if (result != VK_SUCCESS) {
    ...
    return FeErr{
        Error("failed to allocate buffer", ErrorType::RendererVulkanError)};
  }
...
FeExpect<void, Error> BufferManager::ResizeBuffer(Context &ctx, uint64 newSize,
                                                  VulkanBuffer &buffer,
                                                  VkQueue queue,
                                                  VkCommandPool pool) {
  VkBufferCreateInfo bufferInfo = {VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO};
  ...
  VkMemoryRequirements requirements;
  vkGetBufferMemoryRequirements(ctx.device.logicalDevice, newBuffer,
                                &requirements);
  ...
  VkResult result = vkAllocateMemory(ctx.device.logicalDevice, &allocInfo,
                                     ctx.pAllocator, &newMemory);
  if (result != VK_SUCCESS) {
    FLOG_ERROR("failed to allocate new memory on resize");
    return FeErr{Error("failed to allocate new memory on resize",
                       ErrorType::RendererVulkanError)};
  }

  if (auto res = VkCheck(vkBindBufferMemory(ctx.device.logicalDevice, newBuffer,
                                            newMemory, 0));
      !res.has_value()) {
  ...
  auto copyRes = CopyBufferTo(ctx, pool, 0, queue, buffer.handle, 0, newBuffer,
                              0, buffer.totalSize);
...
FeExpect<void, Error>
BufferManager::BindBuffer(Context &ctx, VulkanBuffer &buffer, uint64 offset) {
  if (auto res = VkCheck(vkBindBufferMemory(
          ctx.device.logicalDevice, buffer.handle, buffer.memory, offset));
      !res.has_value()) {
...
FeExpect<void *, Error> BufferManager::LockMemory(Context &ctx,
                                                  VulkanBuffer &buffer,
                                                  uint64 offset, uint64 size,
                                                  uint32 flags) {
  void *data;
  if (auto res = VkCheck(vkMapMemory(ctx.device.logicalDevice, buffer.memory,
...
FeExpect<void, Error>
BufferManager::CopyBufferTo(Context &ctx, VkCommandPool pool, VkFence fence,
                            VkQueue queue, VkBuffer source, uint64 sourceOffset,
                            VkBuffer dest, uint64 destOffset, uint64 size) {
  if (source == VK_NULL_HANDLE || dest == VK_NULL_HANDLE) {
    return FeErr{
        Error("source or dest buffer is null", ErrorType::NullptrException)};
  }
  ...
  return _cmdBufferManager.ImmediateSubmit(
      ctx, pool, queue, fence, [&](VkCommandBuffer cmd) {
        VkBufferCopy region{};
