...
ImageManager::CreateImage(Context &ctx, Image &image, VkImageType imageType,
  ...
  if (auto res = VkCheck(vkCreateImage(ctx.device.logicalDevice, &imgCreateInfo,
                                       ctx.pAllocator, &image.handle));
      !res.has_value()) {
  ...
  vkGetImageMemoryRequirements(ctx.device.logicalDevice, image.handle,
                               &memReqs);
  ...
  if (memType == -1) {
    ...
    return FeErr{Error("required memory type was not found",
  ...
  if (auto res = VkCheck(vkAllocateMemory(ctx.device.logicalDevice, &memInfo,
                                          ctx.pAllocator, &image.deviceMemory));
      !res.has_value()) {
    FLOG_ERROR("failed to allocate memory for image");
    return FeErr{res.error()};
  }

  if (auto res = VkCheck(vkBindImageMemory(
          ctx.device.logicalDevice, image.handle, image.deviceMemory, 0));
      !res.has_value()) {
