...
FeExpect<void, Error>
FrameBufferManager::CreateFrameBuffer(Context &ctx, Renderpass &renderpass,
                                      FrameBuffer *pFrameBuffer, uint32 width,
                                      uint32 height, uint32 attachmentCount,
                                      VkImageView *pAttachments) {
  if (pFrameBuffer == nullptr) {
    FLOG_ERROR("cannot create on nullptr framebuffer");
    return FeErr{Error("attempt to create framebuffer on nullptr",
                       ErrorType::NullptrException)};
  }

  pFrameBuffer->pAttachments = FeCast<VkImageView>(
      _memoryManager.RawAlloc(sizeof(VkImageView) * attachmentCount,
  ...
  if (auto res =
          VkCheck(vkCreateFramebuffer(ctx.device.logicalDevice, &createInfo,
                                      ctx.pAllocator, &pFrameBuffer->handle));
      !res.has_value()) {
...
FeExpect<void, Error>
FrameBufferManager::DestroyFrameBuffer(Context &ctx,
                                       FrameBuffer *pFrameBuffer) {
  vkDeviceWaitIdle(ctx.device.logicalDevice);

  vkDestroyFramebuffer(ctx.device.logicalDevice, pFrameBuffer->handle,
                       ctx.pAllocator);
