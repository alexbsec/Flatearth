...
FeExpect<void, Error> Platform::Initialize() {
  ...
  HWND handle = CreateWindowExA(windowExStyle,
                                "flatearth_window_class",
...
LRESULT Platform::ProcessMessage(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
  ...
  switch (msg) {
    ...
    case WM_SIZE: {
      ...
      _eventManager.FireEvent(SystemEventCode::WindowResized, nullptr, eventCtx);
