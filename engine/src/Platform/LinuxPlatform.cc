...
FeExpect<void, Error> CreateVulkanSurface(PlatformState *platState,
  ...
  VkResult res =
      vkCreateXcbSurfaceKHR(ctx.instance, &createInfo, ctx.pAllocator, &pInternalState->surface);
  if (res != VK_SUCCESS) {
    FLOG_FATAL("Vulkan surface failed to create");
    return FeErr{
        Error("vkCreateXcbSurfaceKHR did not succeeded", ErrorType::PlatformCreateSurface)};
  }
...
FeExpect<void, Error> Platform::Initialize() {
  ...
  // create window
  xcb_void_cookie_t cookie = xcb_create_window(pInternalState->connection,
                                               XCB_COPY_FROM_PARENT,
                                               pInternalState->window,
                                               pInternalState->screen->root,
                                               _xPos,
                                               _yPos,
                                               _width,
                                               _height,
                                               ...
                                               pInternalState->screen->root_visual,
                                               eventMask,
                                               valueList);

  // change title
  xcb_change_property(pInternalState->connection,
                      XCB_PROP_MODE_REPLACE,
                      pInternalState->window,
                      XCB_ATOM_WM_NAME,
                      XCB_ATOM_STRING,
                      8,
                      _applicationName.length(),
  ...
  xcb_intern_atom_cookie_t wmDeleteCookie =
      xcb_intern_atom(pInternalState->connection, 0, cDeleteStr.length(), cDeleteStr.c_str());
  ...
  xcb_change_property(pInternalState->connection,
                      XCB_PROP_MODE_REPLACE,
                      pInternalState->window,
                      pInternalState->wmProtocols,
                      4,
                      32,
                      1,
                      &pInternalState->wmDeleteWin);
  ...
  while ((event = xcb_poll_for_event(pInternalState->connection)) != nullptr) {
    ...
    switch (event->response_type & 0x7f) {
      ...
      case XCB_KEY_RELEASE: {
        ...
        KeySym keySymbol = XkbKeycodeToKeysym(pInternalState->display, (KeyCode)code, 0, level);
        input::Keys key = TranslateKeySymbol((uint32)keySymbol);

        ImGuiIO &io = ImGui::GetIO();
        ImGuiKey imKey = TranslateToImGuiKey((uint32)keySymbol);
        if (imKey != ImGuiKey_None) {
          io.AddKeyEvent(imKey, pressed);
        }
        if (pressed && keySymbol >= 0x20 && keySymbol < 0x80) {
        ...
        if (!processRes.value()) {
          FLOG_WARN("event for key {} was not fired for some reason", (uint32)keySymbol);
        }
      } break;
