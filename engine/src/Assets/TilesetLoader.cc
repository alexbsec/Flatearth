...
FeExpect<Tileset, Error> TilesetLoader::Load(stringv tsxPath) {
  ...
  const string cImagePath =
      (stdfs::path(tsxPath).parent_path() / imageElem->Attribute("source")).string();
  auto texRes =
      _assetManager.LoadTexture(cImagePath).or_error("TilesetLoader: failed to load texture");
  if (texRes.errored()) {
