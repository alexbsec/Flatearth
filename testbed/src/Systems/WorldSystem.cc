...
WorldSystem::WorldSystem(EngineContext &ctx, Orchestrator &orchestrator, scene::SceneId sceneId)
    : _ctx(ctx), _orchestrator(orchestrator), _sceneId(sceneId) {
}

void WorldSystem::Initialize(ecs::Registry &) {
...
void WorldSystem::SpawnEntities(ecs::Registry &registry) {
  _ctx.assets.Tilemap()
  ...
  auto textEntity =
      registry.Spawn()
          .With(ui::UIAnchor{
              .normalizedX = 0.5f, .normalizedY = 0.5f, .scaleX = 0.1f, .scaleY = 0.1f})
          .OwnedBy(_sceneId)
          .Commit();
...
void WorldSystem::Update(ecs::Registry &registry, float32) {
  ...
  for (auto &&[id, audioSrc, worldTag] : registry.ViewOf<scene::AudioSource, WorldTag>()) {
